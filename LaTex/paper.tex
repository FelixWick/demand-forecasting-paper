...
\noindent
As discussed in sec. \ref{sec:ts_confounding}, we do not use any lagged target information in the machine learning model, but apply an individual residual correction on each of its predictions, which accounts for deviations between the EWMA (with a recursive smoothing factor of $0.15$) of the predictions and targets of each product-location combination over the corresponding past. To reflect a realistic replenishment scenario, we use the model to predict the expected demand two days into the future, meaning that we use a lag of two days for the calculation of  both the target EWMA and the prediction EWMA.
...
\caption{\label{fig:ts_res} Time series of mean predictions, before (machine learning prediction) and after (final prediction) residual correction, as well as sales for a specific product-location combination. In addition to the test period in 2016, the last two months of the training period are shown as well, in order to better demonstrate the workings of the residual correction.}
\end{center}
\end{figure}

\noindent
To give an indication of the effectiveness of the two-step approach, i.e., machine learning model without lagged target information and residual correction, we get the following metrics for a model without residual correction, but including the EWMA of the target (grouped by \texttt{store\_id} and \texttt{item\_id}, with a lag of two days, and a recursive smoothing factor of $0.25$) as a feature: MAD: $1.70$ and MSE: $11.04$. The value of the smoothing factor here differs from the one used in the residual correction, because this stricter damping improved the evaluation metrics. Including the day of the week as an additional group by dimension in the EWMA feature worsened the metrics even more.
